...
- `AI_PROVIDER`: Use `bedrock` for the AWS-native architecture or `openai` for the legacy fallback.
- `AWS_REGION`, `AWS_BEDROCK_MODEL_ID`, and `AWS_BEDROCK_EMBEDDING_MODEL_ID`: Required for Bedrock-backed prompts and embeddings.
- `AWS_S3_BUCKET_NAME` and `AWS_S3_DOCUMENT_PREFIX`: Enable the AWS document ingestion pipeline for legal uploads and document processing.
- `OPENAI_API_KEY`: Optional fallback only when `AI_PROVIDER=openai`.

The project is intentionally structured so the same application logic can run on Amazon Bedrock first, while still preserving a working OpenAI fallback during local development and migration.
...
- **AI/ML**: OpenAI GPT-4o, AWS Bedrock, local Ollama fallback, RAG + legal source retrieval
- **Document Processing**: PDF parsing, Word document processing
- **Legal assistant**: Indian legal Q&A, legal safety, source-grounded answer generation, citation verification
